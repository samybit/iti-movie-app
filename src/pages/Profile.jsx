...
const Profile = () => {
	...
	const displayName = userName || user?.displayName || user?.email?.split('@')[0] || t('friend');
	const initials = displayName !== t('friend')
		? displayName.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()
		: 'U';

	const creationDate = user?.metadata?.creationTime 
		? new Date(user.metadata.creationTime).toLocaleDateString(language || 'en-US', { month: 'long', day: 'numeric', year: 'numeric' })
		: 'TBA';
