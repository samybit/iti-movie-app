	...
	if (!user) {
		return (
			<div className='flex flex-col items-center justify-center min-h-[60vh] text-center space-y-6'>
				...
				<h2 className='text-3xl font-black tracking-tighter'>Sign in for your wishlist</h2>
				<p className='text-slate-500 max-w-sm'>
					Keep track of movies and series you want to watch by creating an account.
				</p>
			...
			<div className='flex flex-col md:flex-row md:items-end justify-between gap-6'>
				<div className='space-y-1'>
					<div className='flex items-center gap-3'>
						<h1 className='text-4xl font-black tracking-tight uppercase'>My Library</h1>
					...
					<p className='text-slate-500 font-medium'>
						Your personal collection of must-watch favorites.
					</p>
				</div>
				...
				<div className='rounded-3xl border-2 border-dashed border-slate-200 bg-slate-50/50 p-20 text-center space-y-6'>
					...
					<div className='space-y-2'>
						...
						<p className='text-slate-400 max-w-xs mx-auto text-sm font-medium'>
							Explore thousands of movies and TV shows to build your collection.
						</p>
					</div>
					<Button asChild size='lg' className='rounded-2xl font-bold bg-blue-600 hover:bg-blue-700 shadow-xl shadow-blue-500/20 px-10 gap-2'>
						<Link to='/browse'>Find Movies <ArrowRight size={18} /></Link>
					</Button>
				</div>
			) : filteredWishlist.length === 0 ? (
				<EmptyState description={`No ${filter === 'movie' ? 'movies' : 'series'} in your wishlist yet.`} />
			) : (
